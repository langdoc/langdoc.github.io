...
rmd_info <- plyr::ldply(rmd_files, get_header_attr) %>% left_join(rmd_df) %>% arrange(desc(date)) %>% slice(1)

blog_list_short <- paste(plyr::dlply(rmd_info, .variables = "file", function(x) {
        paste0('<li><a href="', x$html_file, '">', x$title, '</a> by ', x$author, "</li>")
        }
), collapse = "")

blog_list_short <- paste0("<ul>", blog_list_short, "</ul>")

```

Here the newest post in our blog:
