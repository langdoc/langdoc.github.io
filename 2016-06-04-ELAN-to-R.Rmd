...
And this is how it works with our ELAN files, but I want to help others to achieve the same with their corpora!  Of course usually the idea would be to read in the whole corpus, which can be achieved easiest this way:
    ...
    eaf_files <- list.files(path = "path/to/corpus/main/folder", pattern = "eaf$", full.names = T, recursive = T, )
    corpus <- plyr::ldply(eaf_files, FRelan::read_eaf)
...
tier_exists(eaf_file = "/Volumes/langdoc/langs/kpv/kpv_izva20140404IgusevJA/kpv_izva20140404IgusevJA.eaf", 
...
tier_has_content(eaf_file = "/Volumes/langdoc/langs/kpv/kpv_izva20140404IgusevJA/kpv_izva20140404IgusevJA.eaf", 
                 tier = "word@JAI-M-1939", 
                 value = F)
...
## Note to self - find some elegant way to deal with multiple participants here, doesn't work now nicely

```

The trick here is that the result is such a data frame which contains all crucial information from the individual tier. And as we are familiar with the tier hierarchy we can combine the results of different `read_tier()` functions to eventually get the whole file. Only difficulty will be with getting the actual time slots, but on the other hand that is also trivially easy, since that is expressed the same way in each and every ELAN file.

**TO-DO: Explain the tier id relations**
