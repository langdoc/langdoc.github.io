...
So any SoX command that copies the file but does nothing to the content should do quite same, as long as it ends up rewriting the header. `--ignore-length` doesn't really have any real purpose here, but I think otherwise SoX would get stuck to the way how corrupted file seemingly has the length of 0:00.

I'm assuming it would be entirely safe to remove the corrupted files, but actually I have for now kept those as well. I don't know if this makes sense, but I'm anyway quite paranoid about having backups and deleting files always feels bit hazardous. Anyway I wanted to share this procedure here as it is regularly occurring problem, at least for me. I think I've used somewhat different SoX commands in the past as well, but I had to Google this again now when I was working with some older recordings, and ended up with this. This leads of course into more complex question of how we actually store in machine readable format all manipulations which we have done in different software, but this is another topic already…

SoX has tons of parameters that can be tweaked and taken use of. For example, with command:
...
Among other tools, some speaker diarization tools from Aalto University demand this kind of files. I think SoX can also be called from R with [seewave](http://rug.mnhn.fr/seewave/) package, and there seems to be a [Python wrapper](https://pypi.python.org/pypi/sox) as well.
